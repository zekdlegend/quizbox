...
var SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdzemRud3RsbHR6cnJkY2JudXBiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkwNjQ3MjMsImV4cCI6MjA5NDY0MDcyM30.wicKtPGy73STyHQeWo1PVpuwcqvEzVbJX_6TatXlrjs';
...
function loadCategories() {
  ...
  // Load local quizzes from admin
  var localQuizzes = [];
  try {
    var stored = localStorage.getItem('admin_quizzes');
    if (stored) {
      var parsed = JSON.parse(stored);
      localQuizzes = parsed.filter(function(q) {
  ...
  supabaseRequest('GET', 'quizzes', null, 'enabled=eq.true&order=created_at.asc')
    .then(function(data) {
      ...
      var allQuizzes = supabaseQuizzes.concat(localQuizzes);
      if (allQuizzes.length > 0) {
        categories = allQuizzes;
      } else {
